...
class Deez(object):
    ...
    @classmethod
    def download(cls, tracks, numbers = None, fformat = 'mp3', download_path = None, disco = None, overwrite = False, dont_overwrite = False, nmax = 600):
    ...
    def set_config(cls, q):
        ...
        debug(DOWNLOAD_ORIGINAL = cls.DOWNLOAD_ORIGINAL, debug = True)
        debug(DOWNLOAD_DISCOGRAPHY = cls.DOWNLOAD_DISCOGRAPHY, debug = True)
        debug(DOWNLOAD_SINGLE_ORIGINAL_ONLY = cls.DOWNLOAD_SINGLE_ORIGINAL_ONLY, debug = True)
        debug(DOWNLOAD_SINGLE_DISCOGRAPHY_ONLY = cls.DOWNLOAD_SINGLE_DISCOGRAPHY_ONLY, debug = True)
        debug(DOWNLOAD_INTO_ARTIST_FOLDER = cls.DOWNLOAD_INTO_ARTIST_FOLDER, debug = True)
        debug(DOWNLOAD_INTO_SINGLE_FOLDER = cls.DOWNLOAD_INTO_SINGLE_FOLDER, debug = True)
        debug(DOWNLOAD_VARIOUS_ORIGINAL = cls.DOWNLOAD_VARIOUS_ORIGINAL, debug = True)
        debug(DOWNLOAD_VARIOUS_DISCOGRAPHY = cls.DOWNLOAD_VARIOUS_DISCOGRAPHY, debug = True)
        debug(IS_SINGLE = cls.IS_SINGLE, debug = True)
        debug(SHOW_ORIGINAL_ALBUM = cls.SHOW_ORIGINAL_ALBUM, debug = True)
        debug(SHOW_DISCOGRAPHY = cls.SHOW_DISCOGRAPHY, debug = True)
        debug(SHOW_SINGLE_ORIGINAL_ARTIST_ONLY = cls.SHOW_SINGLE_ORIGINAL_ARTIST_ONLY, debug = True)
        debug(SHOW_SINGLE_DISCOGRAPHY_ARTIST_ONLY = cls.SHOW_SINGLE_DISCOGRAPHY_ARTIST_ONLY, debug = True)
        debug(SHOW_VARIOUS_ORIGINAL = cls.SHOW_VARIOUS_ORIGINAL, debug = True)
        debug(SHOW_VARIOUS_DISCOGRAPHY = cls.SHOW_VARIOUS_DISCOGRAPHY, debug = True)
        debug(DOWNLOAD_ALL = cls.DOWNLOAD_ALL, debug = True)
        debug(q_number = q_number, debug = True)
    ...
    @classmethod
    def create_download_path(cls, id, download_path, is_single = False, original_artist = False, single_on_artist_folder = False, single_on_single_folder = False):
        '''
            parameter:
                id = (int) album_id
    ...
    @classmethod
    def create_and_download(cls, disco, ORGARTIST, INTO_SINGLE_FOLDER, INTO_ARTIST_FOLDER, IS_SINGLE, fformat, overwrite, dont_overwrite, download_path, album_numbers = None, q = None):
        download_path0 = download_path
        debug(ORGARTIST = ORGARTIST)
        debug(album_numbers = album_numbers)
        DOWNLOAD_IS_SINGLE = False
        def download(id, ORGARTIST, DOWNLOAD_ORIGINAL_ALBUM_ONLY, INTO_SINGLE_FOLDER, INTO_ARTIST_FOLDER, DOWNLOAD_SINGLE_ONLY, IS_SINGLE, fformat, overwrite, dont_overwrite, download_path, disco):
            while 1:
                try:
    ...
    @classmethod
    def check_config_artist(cls, ftype, artist_id = None, disco = None, artist_name = None):
    ...
    @classmethod
    def filter_disco(cls, disco, artist_name = False, single = False, various = False):
    ...
    @classmethod
    def print_disco_album(cls, disco, ftype, print_list = True):
        artist_id = None
        debug(len_disco = len(disco))
        disco1 = []
        if ftype == 'album':
    ...
    @classmethod
    def print_tracks(cls, album_id = None, disco = None, q = None):
        ...
        if q:
            ...
            return cls.download_interactive(cls.QUERY, cls.DOWNLOAD_PATH, cls.FTYPE, cls.FFORMAT, print_list, cls.OVERWRITE, cls.DONT_OVERWRITE)
    ...
    def search(cls, query, ftype = 'artist', print_list = True, q_search = None, result = None, orgartist = None):
        ...
        debug(query = query, debug = True)
        debug(q = q_search, debug = True)
        debug(print_list = print_list, debug = True)
        debug(orgartist = orgartist)
    ...
    @classmethod
    def download_interactive(cls, query, download_path = None, ftype='artist', fformat='mp3', print_list = True, overwrite = False, dont_overwrite = False, disco = None, artist = None, q = None):
        cls.clean_config()
        cls.QUERY = query
        ...
        if not disco:
            disco, _ = cls.search(query, ftype, print_list, q)
        if not disco:
        ...
        if q and q.isdigit() and int(q) <= len(disco):
            disco_selected = disco[int(q) - 1]
            album_id = disco_selected.get('ALB_ID')
            debug(album_id = album_id)
            ...
            debug(cls_ORGARTIST = cls.ORGARTIST)
            if not album_id:
                album_id = disco_selected.get('id')
            debug(album_id = album_id)
            debug(cls_DIRECT_DOWNLOAD = cls.DIRECT_DOWNLOAD)
            pause()
            if cls.DIRECT_DOWNLOAD:
                while 1:
                    try:
                        tracks = cls.deezer.get_album_tracks(album_id)
                        break
                ...
                debug(download_path = download_path)
                pause()
                cls.download(tracks, None, fformat, download_path, disco[int(q1) - 1], overwrite, dont_overwrite)
            else:
                ...
                if q and q.isdigit() and tracks and int(q) <= len(tracks):
                    ...
                    debug(download_path = download_path)
                    pause()
                    download_status = cls.download(tracks, [int(q2)], fformat, download_path, disco_selected, overwrite, dont_overwrite)
                    ...
                    debug(download_path = download_path)
                    pause()
                    cls.download(tracks, None, fformat, download_path, disco_selected, overwrite, dont_overwrite)
                    ...
                    if "," in q:
                        ...
                        for i in track_number_1:
                            if "-" in i:
                                ...
                                cls.download(tracks, track_number, fformat, download_path, disco_selected, overwrite, dont_overwrite)
                            else:
                                cls.download(tracks, [i], fformat, download_path, disco_selected, overwrite, dont_overwrite)
                    elif "-" in q2:
                    ...
                    if "-" in i:
                        ...
                        while 1:
                            try:
                                ...
                                pass
                        cls.download(tracks, track_number, fformat, download_path, disco[int(q1) - 1], overwrite, dont_overwrite)
