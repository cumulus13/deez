...
class Deez(object):
    ...
    def re_tag(cls, musicfile, more = False):
        ...
        try:
            ...
            if not getattr(f, 'album'):
                ...
                debug(album = album, debug = True)
                f.update({'TALB':album})
            else:
                album = f.album.text[0]
                debug(album = album, debug = True)
            if not getattr(f, 'encodedby'):
                encodeby = TENC(text = album_str)
                debug(encodeby = encodeby, debug = True)
                f.update({'TENC':encodeby})
            else:
                encodeby = f.encodeby.text[0]
                debug(encodeby = encodeby, debug = True)
            
            if getattr(f, "comment"):
                debug(f_comment = f.comment.text[0], debug = True)
                if not isinstance(album, str) or not isinstance(album, bytes):
                    f.comment.text = "{}".format("LICFACE (licface@yahoo.com)")    
                    debug(f_comment = f.comment, debug = True)
                else:
                ...
                debug(album = album, debug = True)
                if not isinstance(album, str) or not isinstance(album, bytes):
                    if not album.text[0] == "SINGLE (more)" or not album.text[0] == "SINGLE":
                        comment = COMM(text = "From Album: {}\n{}".format(album.text, "LICFACE (licface@yahoo.com)"), lang="eng")
                        debug(comment = comment, debug = True)
                    else:
            ...
            debug(f_album = f.album, debug = True)
            debug(saving = "saving ....", debug = True)
            f.save()
            debug(saving = "saving finish ....", debug = True)
    ...
    def download(cls, tracks, numbers = None, fformat = 'mp3', download_path = None, overwrite = False, dont_overwrite = False, download_all_single = False, nmax = 600, debugx = True, pausex = True):
        ...
        for i in numbers:
            ...
            if fformat == 'mp3':
                print(make_colors("downloading:", 'y') + " " + make_colors(tags_separated_by_comma['title'], 'lg'))
                while 1:
                    try:
                ...
                print(make_colors("downloading:", 'y') + " " + make_colors(tags_separated_by_comma['title'], 'lw', 'bl'))
                while 1:
                    try:
